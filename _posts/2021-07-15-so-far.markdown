...
After leading Lambda for some time, I moved to a team called Terminator. Our goal was to completely migrate away from an existing production network to a new one. Google had declared this network legacy, and we wanted to move to a more tenant-based Shared VPC model for various reasons.
